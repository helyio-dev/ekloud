...
export async function getUserBadges(userId: string): Promise<UserBadge[]> {
    ...
    if (error) { console.error('[BADGES] getUserBadges error:', error.message); return []; }
    return (data || []) as UserBadge[];
}

export async function awardBadge(userId: string, badgeId: string): Promise<boolean> {
    const { error } = await supabase.from('user_badges').insert({ user_id: userId, badge_id: badgeId });
    if (error) { if (error.code !== '23505') console.error('[BADGES] awardBadge error:', error.message); return false; }
    return true;
}

export async function checkAndAwardBadges(userId: string, context: {
    xp?: number; streak?: number; completedModules?: number; completedLessons?: number;
    passedExams?: number; friendCount?: number; clan?: string | null; examScore?: number;
}): Promise<string[]> {
    const awarded: string[] = [];
    const give = async (id: string) => { const ok = await awardBadge(userId, id); if (ok) awarded.push(id); };
