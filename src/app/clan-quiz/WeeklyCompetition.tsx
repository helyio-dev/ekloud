...
import { Trophy, Crown, Users, Zap, CheckCircle, XCircle, Loader2, ChevronRight, Calendar, Flame, CalendarDays } from 'lucide-react';
...
export default function WeeklyCompetition({ userClan, defaultTab }: { userClan: SquadId; defaultTab?: 'week' | 'month' }) {
    ...
    const forced = !!defaultTab; // si true, on cache les onglets
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (compIds.length > 0) {
                ...
                (sc || []).forEach((s: any) => {
                    if (!scByComp[s.competition_id]) scByComp[s.competition_id] = [];
                    scByComp[s.competition_id].push(s);
                });

                setScores({
                    week:  compMap['week']  ? (scByComp[compMap['week'].id]  || []).sort((a, b) => b.avg_score_10 - a.avg_score_10) : [],
                    month: compMap['month'] ? (scByComp[compMap['month'].id] || []).sort((a, b) => b.avg_score_10 - a.avg_score_10) : [],
                });
                ...
                (attempts || []).forEach((a: any) => {
                    const type = Object.keys(compMap).find(t => compMap[t as 'week' | 'month']?.id === a.competition_id) as 'week' | 'month' | undefined;
                    if (type) playedMap[type] = a.score;
                });
                setPlayed(playedMap);
            }
        } catch (err) {
            console.error('[COMPETITION] loadData:', err);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const loadQuestions = async () => {
        ...
        // Charger TOUTES les questions actives et en tirer 10 au hasard
        ...
        if (error || !data || data.length === 0) {
            console.error('[COMPETITION] loadQuestions error:', error?.message);
            return;
        }

        // Fisher-Yates shuffle puis on prend les 10 premiers
        const shuffled = [...data];
    ...
    const submitResults = async (final: boolean[]) => {
        ...
        try {
            ...
            console.error('[COMPETITION] submit:', err);
        } finally {
            setIsSaving(false);
        }
    };
    ...
    return (
        <div className="space-y-6">
            {/* Onglets semaine / mois — masqués si page dédiée */}
            {!forced && (
            ...
            {!comp ? (
                ...
                isSaving ? (
                    <div className="flex flex-col items-center py-12 gap-4">
                        ...
                        <p className="text-sm text-text-muted font-medium">Envoi des résultats...</p>
                    </div>
                ) : (
                ...
                <div className="space-y-5">
                    ...
                    <div className={`p-5 rounded-2xl border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${conf.bg} ${conf.border}`}>
                        <div>
                            ...
                            <h3 className={`text-lg font-black ${conf.color}`}>
                                {activeTab === 'week' ? 'Quiz hebdomadaire' : 'Championnat mensuel'}
                            </h3>
                            <p className="text-text-muted/70 text-sm mt-0.5">
                                {comp.question_ids.length > 0 ? `${comp.question_ids.length} questions · Représente ${userClan}` : 'Questions pas encore disponibles'}
                            </p>
                        </div>
                        ...
                        {myScore !== null ? (
                            ...
                            <button onClick={loadQuestions}
                                className="flex items-center gap-2 px-6 py-3 bg-accent hover:bg-accent/90 text-white rounded-xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 shrink-0">
                                {phase === 'done' ? 'Rejouer' : 'Participer'} <ChevronRight className="w-4 h-4" />
                            </button>
                        )}
                    </div>

                    {/* Classement quiz — visible seulement si des clans ont participé */}
                    {clanScores.length > 0 && (
                        <>
...
function ClanLeaderboard({ scores, userClan, type }: { scores: ClanScore[]; userClan: SquadId; type: 'week' | 'month' }) {
    ...
    const full = ALL.map(clan => scores.find(s => s.clan === clan) || { clan, participants: 0, avg_score_10: 0, total_correct: 0 })
        .sort((a, b) => b.avg_score_10 - a.avg_score_10);
        ...
        <div className="space-y-3">
            <div className="flex items-center gap-3">
                ...
                <h3 className="text-[11px] font-black uppercase tracking-[0.3em] text-text-muted">
                    Scores du quiz {type === 'week' ? 'cette semaine' : 'ce mois'}
                </h3>
                {type === 'month' && (
                    <span className="text-[9px] font-bold text-accent/70 uppercase tracking-widest ml-auto">Cumul des 4 semaines</span>
                )}
            </div>
                ...
                return (
                    <div key={s.clan} className={`p-4 rounded-2xl border transition-all ${isMe ? `${c.bg} ${c.border}` : 'bg-surface border-border'}`}>
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className={`font-black text-base shrink-0 ${c.color}`}>
                                {s.participants > 0 ? s.avg_score_10.toFixed(1) : '–'}
                            </span>
                        </div>
