...
/**
 * Carte d'affichage d'un badge — supporte les états gagné, verrouillé, et les tailles.
 */
export default function BadgeCard({ badge, earnedAt, size = 'md', locked = false }: BadgeCardProps) {
    ...
    return (
        <div className={`flex flex-col items-center gap-2 rounded-2xl border transition-all ${sizeClasses.wrap} ${
            locked
            ...
            <div className="text-center space-y-0.5">
                <p className={`font-black uppercase tracking-tight leading-none ${sizeClasses.name} ${locked ? 'text-text-muted' : 'text-text'}`}>
                    {badge.name}
                </p>
                <p className={`font-black uppercase tracking-widest ${sizeClasses.rarity} ${conf.color} opacity-80`}>
                    {conf.label}
                </p>
            </div>
            {earnedAt && size === 'lg' && (
                <p className="text-[9px] text-text-muted/50 font-medium">
